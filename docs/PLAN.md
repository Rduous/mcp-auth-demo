...
- [x] Verify all 8 scenarios in `AGENT_TESTING.md` produce their documented `RESULT:` line — run for real against the live deployment, no human at the consent screen. Found and fixed two real bugs along the way (a deadlock in the headless consent driver, `get_logs` 404ing in production because `docs/` was never copied into the Docker image) plus one in revocation itself (Authlete needs a JWT access token's `jti`, not the raw JWT) — see NOTES.md. Phase 10 is done.
- [x] Friend testing (clean install) surfaced a 9th scenario the original 8 never exercised: step-up across *separate* CLI invocations, not just within one. Root-caused to the SDK's AS metadata being per-process and only discovered on `401`, not `403` — a stored-but-under-scoped token skips straight to `403` in a fresh process and the SDK falls back to the wrong host for the authorize URL. Fixed in `client/main.py` (preload AS metadata unconditionally) and added as Scenario 9 — see NOTES.md.
...
- Adding `check_scope` (to exercise Authlete's native combined scopes+resources check) silently regressed the Phase 4 negative test: Authlete's `action:FORBIDDEN` doesn't say *why*, so a wrong-audience-but-valid-scope token got mislabeled `403 insufficient_scope` instead of `401`. Not caught by code review or the existing suite (Scenario 6 already covered this exact token shape, just never re-run after the regression landed) — only surfaced when directly asked whether any live path actually exercised the quirk. Fixed by checking the resource before trusting any `FORBIDDEN` as a scope verdict.
